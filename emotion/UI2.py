...
import joblib
import torch
from lstm_source import LSTM, collate_fn
...
class window(QWidget):
    ...
    def but1_do(self):
        ...
        output = bayes.predict_proba(vec)
        output1 = output[:, 1]
        # print(output1)
        # print('第一段')
        ...
        vec = vectorizer.transform(words)
        output2 = svm.predict(vec)# return output
        ...
        for s in strs:
            ...
            # 为了解决Tensor 转换效率低下警告，修改为这样
            vectors = np.array(vectors) # 先转换为 numpy array
            vectors = torch.Tensor(vectors) # 再转换为 tensor
            data.append(vectors)
        x, _, lengths = collate_fn(list(zip(data, [-1] * len(strs))))
        with torch.no_grad():
            x = x.to(device)
            outputs = lstm(x, lengths)# 前向传播
            output3 = outputs.view(-1)# 将输出展平
        print(outputs)# outputs = item(outputs)
        predict = output1 * 0.3 + output2 * 0.3 + output3.numpy()*0.4 #return predict
